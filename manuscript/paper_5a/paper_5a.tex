...
Corresponding author: \texttt{ianthelfrich@gmail.com}.
Code and replication data:
\texttt{github.com/ihelfrich/paper5-effective-distance}.
I thank Elizaveta Gonchar for comments and for our joint work on the
companion effective-distance panel. All errors are my own.}}
...
\begin{abstract}
\noindent
The intra-national distance number that applied gravity papers take
from CEPII's GeoDist database comes from the Head-Mayer (2002) closed form
...
$\theta \leq -2$. CEPII evaluates the formula at $\theta = -1$, one unit
...
reduced-form regression slope. Within the model the slope consistently
estimates the composite $\delta(1-\sigma)$, so the choice is best read
as an implicit calibration of the iceberg-cost exponent: $\theta = -1$
with $\sigma \in [4,8]$ commits the user to $\delta$ between $0.14$
and $0.33$, far below the textbook $\delta = 1$. No GeoDist
documentation states that commitment.

Either of two positions rescues the formula, and the literature has
never chosen between them. Adopt $\delta \ll 1$ explicitly, a position
the freight-cost evidence can support but no gravity documentation
currently states. Or keep the textbook calibration $\delta = 1$, where
$\theta = 1-\sigma$ falls in $[-7,-3]$ for $\sigma \in [4,8]$, the
whole range sits below the pole, and the integral the formula
evaluates does not exist as a finite object. CEPII's number is the
harmonic-mean radial coordinate of a country, a well-defined geometric
object either way. What it is not, at the textbook calibration, is the
moment the model asks for.
...
(2016) PPML cookbook, and into applied work wherever intra-national
flows enter the regression.
...
distance in effectively all applied work that includes domestic trade
flows, evaluates equation~\eqref{eq:hm-closed-form} at $\theta = -1$.
...
BACI-CEPII gravity regression on 2010, 2015, and 2020 data, the home-bias
...
The closed form has two singular points, and neither sits at
$\theta = -1$. At $\theta = 0$ the exponent $1/\theta$ is undefined;
the limit exists and equals the geometric mean,
$\dii \to R\, e^{-1/2} \approx 0.607\, R$, so that singularity is
removable. At $\theta = -1$ the formula needs no limit at all: direct
evaluation gives $(2/1)^{-1} R = R/2$, the harmonic mean of the
radial coordinate on the disk. It is finite, defensible,
geometrically meaningful, and the value CEPII evaluates the formula
at. The bilateral one-dimensional formula of
Section~\ref{sec:bilateral-contact} does carry singularities at
$\theta = -1$ and $\theta = -2$, and the two formulas are easy to
conflate on this point; the two-dimensional point-producer form is
regular at $\theta = -1$.
...
$0$ & $606.5$ & Geometric mean (removable limit, $R e^{-1/2}$) \\
$-1.5$ & $396.9$ & Finite, last value tabulated by Head and Mayer (2002) \\
$-1.9$ & $206.7$ & Approaching the pole \\
$-1.99$ & $69.8$ & Approaching the pole ($\dii \sim R\sqrt{(\theta+2)/2}$) \\
$-2.0$ & --- & Pole \\
...
with $3$ seeds and $10{,}000$ atoms per draw.}
...
\begin{itemize}
  ...
  \item Using the panel sweep value at $\theta = -5, d_{\min} = 0.5$ km,
    ...
    observed \$13.1T. The empirical sweep moves in that direction,
    though not the full distance: $\hat\gamma$ at $\theta = -5$,
    $d_{\min} = 0.5$ km is $-9.21 = -5.64 - 3.57$. The realized shift
    of $3.57$ log units against the back-of-envelope $4.9$ reflects
    that the envelope holds everything else fixed, while the
    regression re-equilibrates the distance coefficient and the fixed
    effects across all three years.
\end{itemize}
...
multiplier moves from $0.0036$ to $0.00008$, a factor-of-45 reduction.
